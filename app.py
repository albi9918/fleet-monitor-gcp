...
# Questa singola riga gestisce l'autenticazione sia in locale
# (tramite credentials.json) che su Cloud Run (tramite il service account)
bq_client = bigquery.Client()

# Definisci i riferimenti alla tabella BigQuery
project_id = bq_client.project
dataset_id = "fleet_data"
table_id = "positions"
table_ref = f"{project_id}.{dataset_id}.{table_id}"

# Dizionario per gestire lo stato della conversazione con ogni utente
user_states = {}
...
def handle_location(chat_id, location_data):
    ...
    if not errors:
        ...
        return None  # Non inviare risposte per non intasare la chat
    else:
...
# --- Endpoint API (Webhook per Telegram) ---
...
def send_telegram_message(chat_id, text):
    ...
    # Recupera il token SOLO dalle variabili d'ambiente.
    # Se non c'è, l'app non funzionerà, ed è giusto così.
...
def dashboard():
    ...
    # 1. Recupera i parametri dei filtri dalla URL
    selected_user = request.args.get('username', 'all')
    start_date_str = request.args.get('start_date', '')
    end_date_str = request.args.get('end_date', '')

    # 2. Costruisci dinamicamente la clausola WHERE per le query
    where_conditions = []
    ...
    # 3. Esegui le query usando la clausola WHERE
    ...
    # Query per le rilevazioni dettagliate
    query_rilevazioni = f"""
    ...
    # Query per popolare il menu a tendina con TUTTI gli utenti, indipendentemente dai filtri
    ...
    return render_template(
        ...
        start_date=start_date_str, # Per mantenere il valore nel form
        end_date=end_date_str      # Per mantenere il valore nel form
    )
...
if __name__ == '__main__':
    # Esegui solo in locale per test
